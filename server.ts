...
function calculateFallbackScore(car: typeof FALLBACK_CARS[0], q: RecommendationQuery): number {
  ...
  if (!isBudgetOk) {
    score -= 20; // severe penalty, but still let it display if filters are tight
  }
  ...
  if (q.condition && q.condition !== "any") {
    // If car has years that include "2024", "2025" or "2026", or averageMileage contains "neuf"
    ...
    if (q.condition === "new") {
      if (isNewCar) {
        ...
        score -= 20; // major penalty for used car if user asked for new
      }
    } else if (q.condition === "used") {
      if (!isNewCar) {
        score += 8;
      } else {
        score -= 25; // severe penalty for new car if user asked for used
      }
    }
  }
...
app.post("/api/recommend", async (req, res) => {
  ...
  // If Gemini client is not configured, compute dynamically via custom matching engine
  if (!ai) {
    console.info("[API dev] Gemini API key not found. Executing advanced offline recommendation fallback.");
    
    // Sort and map fallback vehicles based on query parameters
    const matches = FALLBACK_CARS.map(car => ({
      ...car,
      score: calculateFallbackScore(car, query)
    }))
    .sort((a, b) => b.score - a.score)
    .slice(0, 3); // top 3 cars
  ...
  // If Gemini is available, construct prompt and enforce structured schema output
  try {
...
Formate la réponse sous forme de tableau JSON valide d'objets, contenant exactement le schéma demandé. Fais des suggestions réalistes sur le marché de l'automobile en France aujourd'hui. Fais attention à l'estimation des prix en accord avec le marché de l'occasion si le budget est réduit. Donne d'excellentes informations sur la fiabilité mécanique, la vignette Crit'Air et la consommation réelle de carburant.`;
    ...
    // Secure failover: send local mapped data
    const matches = FALLBACK_CARS.map(car => ({
      ...car,
      score: calculateFallbackScore(car, query)
    }))
...
app.post("/api/chat", async (req, res) => {
  ...
  // If no messages or selected car
  ...
  if (!ai) {
    // Advanced offline responsive rule-based helper
    console.info("[API dev] Gemini API key not found. Answering with offline rules engine.");
